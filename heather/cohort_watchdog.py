...
# Paths
# ---------------------------------------------------------------------------
BOT_DIR = os.getenv("BOT_DIR", "/data")
LOG_BASE = os.getenv("LOG_DIR", "/app/logs")
LOG_PATHS = [
    Path(BOT_DIR) / "logs" / "heather_bot.log",
    Path(LOG_BASE) / "heather_bot.log",
]
# Include rotated logs from LOG_BASE (the primary log location)
ROTATED_LOG_PATHS = [
    Path(LOG_BASE) / "heather_bot.log.1",
    Path(LOG_BASE) / "heather_bot.log.2",
    Path(LOG_BASE) / "heather_bot.log.3",
]
DISCLOSURE_FILE = Path(BOT_DIR) / "ai_disclosure_shown.json"
TIP_FILE = Path(BOT_DIR) / "tip_history.json"
REPORT_DIR = Path(os.getenv("REPORT_DIR", str(Path(BOT_DIR) / "reports")))
