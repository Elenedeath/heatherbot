"""
Daily Story Generator & Discord Poster
==========================================================
Generates a first-person daily journal entry.
Incorporates real weather, day of week, Current locations, and cultural references.
...
Usage:
    ...
    python daily_story_poster.py --loop       # Post daily at 9am
...
STORY_DIR = Path(LOG_DIR) / "discord_stories"
...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler(Path(LOG_DIR) / "daily_story.log", encoding="utf-8"),
        logging.StreamHandler(),
    ],
)
log = logging.getLogger("daily_story")

LLM_HOST = os.getenv("TEXT_HOST", "127.0.0.1")
LLM_PORT = os.getenv("TEXT_PORT", "1234")
LLM_URL = f"http://{LLM_HOST}:{LLM_PORT}/v1/chat/completions"
DISCORD_TOKEN = os.getenv("DISCORD_TOKEN", "")
STORIES_CHANNEL_NAME = os.getenv("STORIES_CHANNEL_NAME", "stories")
TELEGRAM_USERNAME = os.getenv("TELEGRAM_USERNAME", "UberMommy")
...
- Sign off: kiss emoji then Heather, then on next line "*Find me on Telegram: [{TELEGRAM_USERNAME}](https://t.me/{TELEGRAM_USERNAME})*"
...
def main():
    parser = argparse.ArgumentParser(description="Daily Story Generator + Discord Poster")
